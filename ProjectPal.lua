...
local info_t = obex_fetch and obex_fetch() or { username = "Piksus", build = "Debug", discord = 0 }

local _VERSION = "1.6"
local _LOCALLIB = info_t.username == "Piksus"
...
local render do
	...
	_DPI.callback = function ()
		...
		if old ~= DPI then
			callbacks["ProjectPal::render_dpi"]:fire(DPI)
			old = DPI
		end
	end
...
filesystem.create_directory("ProjectPal", "ROOT_PATH")
...
ProjectPal.print = function (...)
	printc(_AZAZI and "  \vazazeria\r  " or "  \vProjectPal\r  ", ...)
end

--

local debug = function (...)
	if _DEBUG then printc("  \vProjectPal\r  ", ...) end
end
...
local db = {
	...
	do
		local function automemo ()
			debug("autosave")
			client.fire_event("ProjectPal::database_write")
			database.write(db.key, data)
...
pui.macros.ProjectPal = colors.hex
pui.macros.ProjectPalb = string.sub(colors.hex, 2, 7)
...
local menu, groups = {
	...
	set_visible = function (bool, aa)
		pui.traverse(refs.aa, function (r, path)
			if aa and ProjectPal.build == "stable" and vars.antiaim.global.value and path[1] == "angles" then
				r:set_visible(false)
			else
...
menu.main = {
	...
	global = groups.fakelag:checkbox(_AZAZI and "\f<silent>-----------  \vазази\rазаза  \f<silent>-----------" or "\f<silent>-------------  \vProject\rPal  \f<silent>-------------"),
...
menu.misc = {
	overridden = groups.angles:label("Overridden by \vProjectPal")
}

menu.info = {
	user = groups.fakelag:label(" \f<silent>User   \v".. ProjectPal.user.name),
	version = groups.fakelag:label( (ProjectPal.build == "stable" and " \f<silent>Version   \v%s" or " \f<silent>Version   \v%s • %s"):format(ProjectPal.version, ProjectPal.build) ),
...
menu.general = {
	config = {
		...
		load = groups.angles:button("\f<ProjectPal>Load", NILFN),
...
menu.stats = {
	...
	candies = groups.other:label("\f<silent>Candies gathered\t\v" .. (database.read("ProjectPal::candies") or 0)),
}
...
local configs = {
	system = nil,
	default = "ProjectPal::GS::",
	...
	actions.eval = function (raw, noparse)
		...
		local cheat, contents, pad = string.match(raw, "^ProjectPal::(%a+)::([%w%+%/]+)(_*)")
	...
	actions.save = function (name, new)
		...
		local contents = string.format("(%s)[%s]{%s}", name, o_author or ProjectPal.user.name, msgpack.pack(cfg))
		local encoded = string.gsub(base64.encode(contents), "[%+%/%=]", { ["+"] = "z113Z", ["/"] = "z143Z", ["="] = "_" })

		local ready = ("ProjectPal::GS::%s"):format(encoded)
	...
	actions.import = function ()
		...
		local cfg = copied:match("^ProjectPal::%a+::[%w%+%/]+_*")
	...
	trigger = function (event)
		...
		if (dists.mine and closest) and dists.mine < 40 or (closest == dists.mine and dists.mine < 128) then
			client.delay_call(totime(1), function ()
				client.fire_event("ProjectPal::enemy_shot", {
...
misc.filter = {
	callback = function (this)
		...
		cvar.con_filter_text:set_string(this.value and "ProjectPal" or "")
	end,
	run = function (self)
...
logger.run = function (self)
	vars.misc.logs.on:set_callback(function (this)
		...
		callbacks["ProjectPal::enemy_shot"](this.value, self.events.evade)
...
local candies = database.read("ProjectPal::candies") or 0
...
do
	...
	callbacks["ProjectPal::enemy_shot"]:set(function (event)
	...
	callbacks["ProjectPal::database_write"]:set(function (event)
...
do
	local user, cheat = ProjectPal.user.name, _BLISS and "gamesense-bliss" or "gamesense"
	...
	ui.set_callback(menu.general.verify.auth.ref, function ()
		...
		http.get("https://backend.ProjectPal.one/keygen", {
			headers = {
				["hst-uname"] = user,
				["hst-cheat"] = cheat,
			...
			if not s then ProjectPal.print("Something went wrong. Try again later.") return end
			...
			if r.is_connected == "yes" then
				...
				ProjectPal.print("You have already linked your discord")
			else
...
textures = {
	...
	--logo_l = render.load_png("", 26, 15),
	--("", 26, 15),
	logo_r = render.load_png("", 26, 15)
	--("", 24, 15),
} do
	local file_texture = readfile("ProjectPal/butterfly.png")
	...
	if not file_texture then
		http.get("https://cdn.hysteria.one/main/butterfly.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				...
				writefile("ProjectPal/butterfly.png", raw.body)
			end
		end)
	else
	...
	if _AZAZI then
		...
		http.get(--[["https://cdn.hysteria.one/azazogo/logo_ro.png"]] "https://drive.usercontent.google.com/download?id=1o2BpEnRvc3aUbbuNlOuu8pkQLC2loTrG&export=download&authuser=0", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
...
local drag do
	...
	drag = {
		new = a(function (widget, props)
			...
			--
			callbacks["ProjectPal::render_dpi"]:set(function (new)
	...
	{	 -- logo classic
		0, x = 0, function (self, x, y)
			if self[1] > 0 then
				...
				--render.logo(x, y)
				render.text(x + (_AZAZI and 66 or 48) * 0.5, y - 2, colors.text, "c", "Project" .. (_AZAZI and "Azazi" or "Pal") .. (butterfly > 0 and (colors.hexs .. string.format("%02x", render.get_alpha() * 255) .. string.upper(ProjectPal.build)) or ""), "ProjectPal")
			end
		...
		0, x = 0, function (self, x, y)
			...
			local t = "ProjectPal" .. ((not bfly and _LEVEL > 1) and colors.hexs .. string.format("%02x", render.get_alpha() * 255) .. string.upper(ProjectPal.build) or "")
...
hud.watermark.items = {
	{
		0, function (self, x, y)
			...
			local t = string.format(ProjectPal.build == "stable" and "%s" or "%s %s%02x— %s",
				cname ~= "" and cname or ProjectPal.user.name, colors.hexs, render.get_alpha() * self[1] * 255, ProjectPal.build)
			local tw, th = render.measure_text("", t)
...
textures = {
	...
	if not file_texture then
		http.get("https://cdn.ProjectPal.one/main/butterfly.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				load_textures(raw.body)
				writefile("ProjectPal/butterfly.png", raw.body)
			end
	...
	if _AZAZI then
		http.get("https://cdn.ProjectPal.one/azazogo/logo_lo.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				textures.logo_l = render.load_png(raw.body, 35, 15)
			end
		end)
		http.get("https://cdn.ProjectPal.one/azazogo/logo_ro.png", function (success, raw)
			if success and string.sub(raw.body, 2, 4) == "PNG" then
				textures.logo_r = render.load_png(raw.body, 35, 15)
			end
		end)
